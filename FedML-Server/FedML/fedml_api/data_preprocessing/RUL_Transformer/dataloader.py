    ...
    def load_dataset(self, config):
        if self.mode == "train":
            train_df = pd.read_csv(
                config["data_path"] + f"train{config['client_id']}.csv"
            )
            ...
            label_gen = [
                gen_labels(
                    train_df[train_df["id"] == id], self.config["l_win"], ["RUL"]
                )
                for id in train_df["id"].unique()
            ]
            ...
            test_df = pd.read_csv(
                config["data_path"] + f"test{config['client_id']}.csv"
            )
            ...
            seq_array_test_last = [
                test_df[test_df["id"] == id][sequence_cols].values[-config["l_win"] :]
                for id in test_df["id"].unique()
                if len(test_df[test_df["id"] == id]) >= config["l_win"]
            ]

            seq_array_test_last = np.asarray(seq_array_test_last).astype(np.float32)

            y_mask = [
                len(test_df[test_df["id"] == id]) >= config["l_win"]
                for id in test_df["id"].unique()
            ]
