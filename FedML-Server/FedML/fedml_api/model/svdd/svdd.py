...
class SVDD():
    ...
    # test_SHAP (viết hàm test_SHAP đưa vào class, nhưng không được xóa hàm test cũ đi)
    def test_SHAP(self, data):
        ...
        predictedlabel = np.ones((n,1))
        fault_index = np.where(distance > self.model["radius"])[1][:]
        predictedlabel[fault_index] = -1
        #predictedlabel = predictedlabel.reshape(-1)
        return predictedlabel.flatten()
    ...
    def train(self, data, label):
        ...
        if self.parameters["option"]["display"] == 'on':
            ...
            print('threshold        = %.4f'     % (self.model["radius"]))
            print('\n')
